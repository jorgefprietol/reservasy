...
Agenda Capacitaciones Yanbal was designed to make authentication pluggable so that new credential stores can be without changes to the Agenda Capacitaciones Yanbal source code.
Agenda Capacitaciones Yanbal ships with an LDAP authentication plugin.  This should serve as a good example because it is sufficiently complex and covers a typical scenario.
...
- Within this source file, require the Authentication namespace and implement the IAuthentication interface
	...
	?>
- In the Agenda Capacitaciones Yanbal configuration file, set $conf['settings']['plugins']['Authentication'] to the folder name. For example, $conf['settings']['plugins']['Authentication'] = 'Ldap';
...
This method is only called if the result of Validate() was true.  Typically, this method would synchronize data between the source store and Agenda Capacitaciones Yanbal,
although this is not required if the user account data already exists in Agenda Capacitaciones Yanbal.
...
The Login method should always call to the decorated Authentication classes Login() method. This ensures that any functionality required by Agenda Capacitaciones Yanbal is executed.
