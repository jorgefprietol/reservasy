...
<h1 xmlns="http://www.w3.org/1999/html">Amministrazione di Agenda Capacitaciones Yanbal</h1>
...
<p>All'installazione di Agenda Capacitaciones Yanbal, un calendario di default viene generato automaticamente. Dal menu "Prenotazioni" &egrave; possibile accedere alla modifica di tutte le caratteristiche delle prenotazioni.</p>
...
<p>Una risorsa di Agenda Capacitaciones Yanbal pu&ograve; essere qualsiasi cosa si possa prenotare come, ad esempio, un'aula o un equipaggiamento. Ogni risorsa deve essere assegnata ad un calendario per essere prenotabile. La risorsa eredita il layout di prenotazione del calendario al quale appartiene.</p>
...
<p>I Gruppi, in Agenda Capacitaciones Yanbal permettono di organizzare gli utenti, gestire i permessi di accesso alle risorse e definire ruoli all'interno dell'applicazione.</p>
...
<p class="setting"><span>$conf['settings']['enable.email']</span>Se Agenda Capacitaciones Yanbal &egrave; abilitato, o meno, ad inviare email.</p>

<p class="setting"><span>$conf['settings']['default.language']</span>Lingua impostata per default per tutti gli utenti. Si pu&ograve; impostare qualsiasi lingua contenuta nella cartella 'lang' di Agenda Capacitaciones Yanbal.</p>

<p class="setting"><span>$conf['settings']['script.url']</span>L'URL pubblico assoluto della cartella di Agenda Capacitaciones Yanbal sul server. Questo dovrebbe essere la directory Web che contiene i file come bookings.php e calendar.php. Se questo indirizzo viene lasciato senza un protocollo specificato allora il protocollo (http vs https) viene rilevato automaticamente.</p>

<p class="setting"><span>$conf['settings']['image.upload.directory']</span>L'indirizzo della cartella nella quale archiviare le immagini delle risorse. Questa cartella deve avere i permessi di scrittura (755 &egrave; suggerito). Questo indirizzo pu&ograve; essere sia relativo alla cartella di Agenda Capacitaciones Yanbal, sia assoluto.</p>
...
<p class="setting"><span>$conf['settings']['logout.url']</span>L'indirizzo web al quale l'utente verr&agrave; indirizzato all'uscita da Agenda Capacitaciones Yanbal. Il default &egrave; la pagina di login.</p>
...
<p class="setting"><span>$conf['settings']['database']['name']</span>Nome del database di Agenda Capacitaciones Yanbal</p>
...
<p class="setting"><span>$conf['settings']['api']['enabled']</span>Se, o meno, le API di RESTful di Agenda Capacitaciones Yanbal devono essere abilitate. Ulteriori informazioni sui prerequisiti per l'utilizzo delle API sono rintracciabili nel file readme_installation.html file. Le opzioni sono: 'true' o 'false'.</p>
...
<p>I plugin possono avere il proprio file di configurazione. Per LDAP, rinominare o copiare /plugins/Authentication/Ldap/Ldap.config.dist in /plugins/Authentication/Ldap/Ldap.config e editare tutti i valori che sono applicabili all'ambiente in cui &egrave; inserito Agenda Capacitaciones Yanbal.</p>
