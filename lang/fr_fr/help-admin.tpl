...
Ce fichier fait parti de Agenda Capacitaciones Yanbal.

Agenda Capacitaciones Yanbal est un logiciel libre : vous pouvez le redistribuer et/ou le
...
Agenda Capacitaciones Yanbal est distribué dans l'espoir d'être utile mais
...
avec Agenda Capacitaciones Yanbal. si ce n'est pas le cas consultez <http://www.gnu.org/licenses/>.
*}
{include file='globalheader.tpl'}
<h1>Administration de Agenda Capacitaciones Yanbal </h1>
...
<p>Les Ressources dans Agenda Capacitaciones Yanbal peuvent être tout ce que vous voulez (Salles, équipements, ...). Chaque ressource doit être liée à un planning pour être réservable. La ressource héritera donc des créneaux horaires de réservabilité définis pour le planning.
</p>
...
<p>Les quotas permettent de limiter l'utilisation d'une ressource. Ce système dans Agenda Capacitaciones Yanbal est très flexible, en autorisant de limiter les réservations en durée et en nombre.
...
<p class="setting"><span>$conf['settings']['server.timezone']</span>C'est le fuseau horaire du serveur qui héberge Agenda Capacitaciones Yanbal.
...
<p class="setting"><span>$conf['settings']['enable.email']</span>Autorise l'envoi d'emails (ou non) depuis Agenda Capacitaciones Yanbal.
</p>

<p class="setting"><span>$conf['settings']['default.language']</span>Langue par défaut pour tous. Ce peut être toute langue disponible dans le répertoire "lang" de Agenda Capacitaciones Yanbal.</p>

<p class="setting"><span>$conf['settings']['script.url']</span>L'URL publique complète (racine de l'instance de Agenda Capacitaciones Yanbal). Devrait être le repertoire Web qui contient les fichiers bookings.php, calendar.php,  ...</p>
...
<p class="setting"><span>$conf['settings']['css.extension.file']</span>URL relative ou absolue pointant sur un fichier CSS supplémentaire à inclure. On peut ainsi "Surcharger" le CSS en place pour créer son propre style voire son propre thème. A laisser vide si vous voulez utiliser Agenda Capacitaciones Yanbal tel quel.</p>
