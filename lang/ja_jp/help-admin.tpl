...
<h1 xmlns="http://www.w3.org/1999/html">Agenda Capacitaciones Yanbal 管理</h1>
...
<p>
Agenda Capacitaciones Yanbal をインストールするとき、何もしなくてもデフォルトスケジュールが作成されます。
...
<p>Agenda Capacitaciones Yanbal での「リソース」とは、部屋や設備のような予約制にしたいあらゆるもののことです。
...
Agenda Capacitaciones Yanbal の量制限システムは柔軟にできていて、予約時間と回数で制限をかけることができます。
また、量制限は「重なり」ます。
...
<p>お知らせは Agenda Capacitaciones Yanbal のユーザーに注意事項を知らせる簡単な方法です。
...
<p>Agenda Capacitaciones Yanbal でのグループはユーザーを組織化し、リソース使用の可否を決定し、アプリケーション内での役割を定めます。</p>
...
Agenda Capacitaciones Yanbal では予め汎用のレポートを用意しており、リストや図として表示すること、CSVとして出力すること、印刷することができます。
...
更に、サーバーのスケジュールタスクが /Agenda Capacitaciones Yanbal/Jobs/sendreminders.php を実行するようにしなくてはなりません。</p>

<p>Linux の場合は cron ジョブを使えるでしょう。実行するコマンドは <span class="note">php</span> の後ろに
Agenda Capacitaciones Yanbal/Jobs/sendreminders.php のフルパスを付けたものです。このサーバーの sendreminders.php へのフルパスは
...
Agenda Capacitaciones Yanbal/Jobs/sendreminders.php のフルパスを指定したものです。</p>
...
<p class="setting"><span>$conf['settings']['enable.email']</span>Agenda Capacitaciones Yanbal がメールを送信するか否か</p>

<p class="setting"><span>$conf['settings']['default.language']</span>ユーザーのデフォルトの言語。Agenda Capacitaciones Yanbal のlangディレクトリにある言語ならどれでも可</p>

<p class="setting"><span>$conf['settings']['script.url']</span>Agenda Capacitaciones Yanbal インスタンスのルートへの完全な公開URL。
...
このディレクトリに書き込みできる(755とか)必要がある。この指定は完全ディレクトリ名(フルパス)またはAgenda Capacitaciones Yanbalのルートディレクトリからの相対パスのいずれでもよい。</p>
...
これはデフォルトのスタイルを上書きするために使うことができ、完全なテーマとして使用することもできる。 Agenda Capacitaciones Yanbalのスタイルを拡張しないなら空白のままにしておく。</p>
...
<p class="setting"><span>$conf['settings']['uploads']['reservation.attachment.path']</span>添付ファイルを保存するディレクトリのファイルシステム上の絶対または(Agenda Capacitaciones Yanbal ディレクトリのルートからの)相対パス。ディレクトリにPHPが書き込み可能(755とか)でなくてはならない。デフォルトは uploads/reservation</p>
...
<p class="setting"><span>$conf['settings']['database']['name']</span>Agenda Capacitaciones Yanbal 使うデータベース名</p>
...
<p class="setting"><span>$conf['settings']['api']['enabled']</span>Agenda Capacitaciones Yanbal で RESTful API を有効にするかどうか。
